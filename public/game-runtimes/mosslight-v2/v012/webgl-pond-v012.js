...
export function createPondRenderer(G){
  ...
  function collect(){
    ...
    const p=state.player;if(p){
      const rig=window.SylvariaCharacterRig?.pose?.(p),frogSize=Math.max(48,p.r*3.05);
      if(rig){const body=rig.body;emitShadow(p.x,p.y+p.r*.7,body.w*.82,Math.max(8,body.h*.22),6.2);emit('frog',body.x,body.y,body.w,body.h,{rot:body.rotation,layer:6.3,foot:body.foot,tint:p.shieldCharges>0?[.86,1,1,1]:[1,1,1,1]});if(p.shieldCharges>0)emit('spark',body.x,body.y,Math.max(body.w,body.h)*1.35,Math.max(body.w,body.h)*1.35,{layer:6.35,foot:p.y,tint:[.62,.9,1,.28]})}
      else{const d=DIRS[p.cutDirection]||DIRS.right,angle=Math.atan2(d.y,d.x);emitShadow(p.x,p.y+p.r*.7,frogSize*.82,frogSize*.22,6.2);emit('frog',p.x,p.y-p.pose*1.5,frogSize,frogSize,{rot:angle,layer:6.3,foot:p.y+p.r*.68,tint:p.shieldCharges>0?[.86,1,1,1]:[1,1,1,1]});if(p.shieldCharges>0)emit('spark',p.x,p.y,frogSize*1.35,frogSize*1.35,{layer:6.35,foot:p.y,tint:[.62,.9,1,.28]})}
    }
