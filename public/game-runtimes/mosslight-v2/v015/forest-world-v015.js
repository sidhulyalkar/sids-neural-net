...
function applyRoom(){
  activeBiome=biomeForDepth();forestPatches=0;regrown=0;const depth=state.worldDepth||1,bp=G.roomBlueprint(depth);
  if(state.room){if(bp?.v015Roster){state.room.title=bp.title;state.room.subtitle=`${activeBiome.label.toLowerCase()} · ${bp.subtitle}`}else{const names=TITLES[activeBiome.id];state.room.title=names[(depth-1)%names.length];state.room.subtitle=`${activeBiome.label.toLowerCase()} · carve visibility · survive the clearcut`}}
  ...
  for(const e of state.enemies)if(!e.v015Name)e.v015Name=ENEMY_NAMES[e.type]||e.kineticType||e.type;
...
function drawBrushFog(){const boon=state.v015RunBoons?.fogMultiplier||1,burst=state.player?.v015VisionBurst>0?.42:1,fogScale=boon*burst;for(const p of state.terrain){if(!p.active||p.type!=='grass')continue;const ratio=brushRatio(p);if(ratio<.06)continue;const r=(p.r||45)*1.15,g=ctx.createRadialGradient(p.x,p.y,r*.15,p.x,p.y,r);g.addColorStop(0,`rgba(4,16,9,${(.08+.14*ratio)*fogScale})`);g.addColorStop(.62,`rgba(7,25,13,${(.11+.18*ratio)*fogScale})`);g.addColorStop(1,'rgba(4,13,8,0)');ctx.fillStyle=g;ctx.fillRect(p.x-r,p.y-r,r*2,r*2)}}
