...
function drawActive(p){
  const d=p.dash;if(!d?.v015Cutstep)return;const color=COLORS[d.v015Technique]||COLORS.carve,lead=window.SylvariaCutstep?.config?.bladeLead||0,tipX=p.x+d.dir.x*lead,tipY=p.y+d.dir.y*lead;
  ctx.save();ctx.lineCap='round';const g=ctx.createLinearGradient(d.sx,d.sy,tipX,tipY);g.addColorStop(0,'rgba(213,255,190,.05)');g.addColorStop(.82,color);g.addColorStop(1,'rgba(248,255,221,.98)');ctx.strokeStyle=g;ctx.lineWidth=d.v015Technique==='crosscut'?8:6;ctx.beginPath();ctx.moveTo(d.sx,d.sy);ctx.lineTo(tipX,tipY);ctx.stroke();ctx.fillStyle='rgba(248,255,221,.9)';ctx.beginPath();ctx.arc(tipX,tipY,3.2,0,Math.PI*2);ctx.fill();
...
function drawAim(p){
  ...
  const tech=prospectiveTechnique(p,aim),config=system.config,distance=tech==='thrust'?config.thrustDistance:tech==='reversal'?80:config.distance,tx=p.x+aim.x*distance,ty=p.y+aim.y*distance,bx=tx+aim.x*(config.bladeLead||0),by=ty+aim.y*(config.bladeLead||0),color=COLORS[tech]||COLORS.carve,ready=(p.v015Segments||0)>=1;
  ctx.save();ctx.lineCap='round';ctx.setLineDash([5,7]);ctx.strokeStyle=ready?color:'rgba(190,194,170,.32)';ctx.globalAlpha=ready ? .58 : .32;ctx.lineWidth=1.8;ctx.beginPath();ctx.moveTo(p.x+aim.x*(p.r+8),p.y+aim.y*(p.r+8));ctx.lineTo(tx,ty);ctx.stroke();ctx.setLineDash([]);ctx.globalAlpha=ready ? .84 : .42;ctx.strokeStyle=color;ctx.lineWidth=2;ctx.beginPath();ctx.arc(tx,ty,6,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(tx-aim.y*7,ty+aim.x*7);ctx.lineTo(tx+aim.y*7,ty-aim.x*7);ctx.stroke();ctx.globalAlpha=ready ? .95 : .42;ctx.lineWidth=3.4;ctx.beginPath();ctx.moveTo(tx,ty);ctx.lineTo(bx,by);ctx.stroke();ctx.fillStyle=color;ctx.beginPath();ctx.arc(bx,by,2.8,0,Math.PI*2);ctx.fill();
...
window.SylvariaCutstepPresentation=Object.freeze({version:CUTSTEP_PRESENTATION_VERSION,canvas:overlay,snapshot:()=>({version:CUTSTEP_PRESENTATION_VERSION,renderedFrames,aimFrames,pathFrames,logical:{width:W,height:H},aim:state.player&&window.SylvariaCutstep?.currentAim?.(state.player),segments:q(state.player?.v015Segments||0),activeTechnique:state.player?.dash?.v015Technique||null,bladeLead:window.SylvariaCutstep?.config?.bladeLead||0})});
