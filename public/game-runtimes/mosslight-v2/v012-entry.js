...
if(playtest){
  ...
  playtest.snapshot=()=>{const snap=base();return{...snap,presentationVersion:PRESENTATION,visual:{...(snap.visual||{}),presentationVersion:PRESENTATION,frogPond:true,webgl2Pond:renderer.ready,tongueAttack:true,insectEnemies:true,pondEnvironment:true,renderMode:renderer.snapshot().mode}}};
}
const visual=window.SylvariaVisualSystem;
if(visual){const base=visual.snapshot.bind(visual);visual.presentationVersion=PRESENTATION;visual.snapshot=()=>({...base(),presentationVersion:PRESENTATION,frogPond:true,webgl2Pond:renderer.ready,tongueAttack:true,insectEnemies:true,pondEnvironment:true,renderMode:renderer.snapshot().mode})}
window.Sylvaria012=G;
window.SylvariaPondRenderer=renderer;
window.SylvariaPresentation012=Object.freeze({version:PRESENTATION,engineVersion:ENGINE,roomName:renderer.displayRoomName});
