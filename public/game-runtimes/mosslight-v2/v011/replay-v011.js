...
function push(action){if(!active||state.mode!=='playing')return;if(tick+1>MAX_TICKS){invalidate('duration limit');return}if(events.length>=MAX_EVENTS){invalidate('input event limit');return}const targetTick=tick+1;events.push({tick:targetTick,action});}
function reset(){tick=0;active=true;eligible=true;overflowReason=null;events=[];finalized=null;recordedHeld.clear();runSerial++;}
