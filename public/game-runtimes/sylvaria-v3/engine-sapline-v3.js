...
// The player keeps the velocity generated by the spring, so good release timing
// naturally chains into wall launches, Downstrikes and Canopy Step.
export class SylvariaEngine extends FeelEngine{
  freshStats(){return{...super.freshStats(),saplineAttaches:0,saplineLaunches:0,maxSaplineSpeed:0}}
  ...
  startSapline(){
    ...
    const rest=clamp(target.dist*SAPLINE.attachRestRatio,SAPLINE.minLength,target.dist);
    p.sapline={anchorId:target.anchor.id,rest,attachDistance:target.dist,extension:Math.max(0,target.dist-rest),peakExtension:Math.max(0,target.dist-rest),age:0};
    p.onGround=false;p.groundId=null;p.coyote=0;p.wallDir=0;p.wallId=null;
  ...
  releaseSapline(launch=true){
    ...
    if(launch&&pos&&tether.age>=1/30){
      ...
      if(pos.y<p.y-24&&extension>18)p.vy=Math.min(p.vy,-Math.min(430,SAPLINE.releaseUpFloor+boost*.28));
      const speed=Math.hypot(p.vx,p.vy);
      if(speed>SAPLINE.maxReleaseSpeed){const k=SAPLINE.maxReleaseSpeed/speed;p.vx*=k;p.vy*=k}
      if(extension>12){this.state.stats.saplineLaunches++;this.emit('saplineLaunch',{anchorId:tether.anchorId,extension,speed:Math.hypot(p.vx,p.vy)})}
    }
    p.lastSaplineAnchor=tether.anchorId;p.sameAnchorCooldown=SAPLINE.sameAnchorCooldown;p.saplineCooldown=SAPLINE.detachCooldown;p.sapline=null;p.saplineCandidate=null;
  ...
  updateSapline(dt){
    ...
    const springAccel=clamp(extension*SAPLINE.spring-radialVelocity*SAPLINE.damping,0,SAPLINE.maxAccel);
    ...
    tether.extension=extension;tether.peakExtension=Math.max(tether.peakExtension||0,extension);
  ...
  respawnFromFall(){if(this.state.player.sapline)this.releaseSapline(false);super.respawnFromFall();const p=this.state.player;p.sapline=null;p.saplineCandidate=null;p.saplineCooldown=0;p.sameAnchorCooldown=0}
  ...
  snapshot(){
    ...
    return{
      ...snap,
      player:{...snap.player,sapline:tether?{anchorId:tether.anchorId,rest:tether.rest,extension:tether.extension,peakExtension:tether.peakExtension,age:tether.age,anchor:pos}:null,saplineCandidate:p.saplineCandidate},
