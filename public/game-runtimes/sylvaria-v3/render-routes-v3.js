...
export class SylvariaRenderer extends MotionRenderer{
  ...
  drawBoss(b){
    ...
    const ctx=this.ctx,t=this.engine.state.time,open=b.guard<=0,clampCount=Math.max(3,b.maxGuard||3);
    ctx.save();ctx.translate(b.x,b.y);ctx.globalAlpha=b.hitFlash>0 ? .58 : 1;
    // Crown Girdler: a mechanical parasite whose visible clamps exactly mirror guard state.
    ...
    for(let i=0;i<clampCount;i++){const mix=clampCount===1?.5:i/(clampCount-1),a=lerp(-2.48,.18,mix),intact=i<b.guard,px=Math.cos(a)*76,py=Math.sin(a)*76;ctx.save();ctx.translate(px,py);ctx.rotate(a+Math.PI/2);ctx.fillStyle=intact?'#2e3935':'#151b19';ctx.fillRect(-15,-11,30,22);ctx.strokeStyle=intact?COLORS.danger:'rgba(120,98,72,.35)';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-12,7);ctx.lineTo(-25,20);ctx.moveTo(12,7);ctx.lineTo(25,20);ctx.stroke();if(intact){ctx.fillStyle='#d68f47';ctx.beginPath();ctx.arc(0,-2,4+Math.sin(t*5+i)*.7,0,TAU);ctx.fill()}ctx.restore()}
