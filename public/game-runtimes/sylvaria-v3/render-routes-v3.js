...
import{VIEW,TAU,COLORS,MOVE,SAPLINE}from'./config-v3.js';
...
export class SylvariaRenderer extends MotionRenderer{
  drawPlatform(platform){
    ...
    // Tapered twigs make the collision span visually honest without reading as a shelf.
  ...
  drawBarkRail(rail){
    ...
    // Moss always sits on the geometric upper surface. Route class never changes color.
  ...
  drawGuardian(p){
    ...
    // Velocity-vector cloak: the visual body can be larger than the compact hitbox.
    ...
    if(wall){ctx.moveTo(-5,14);ctx.lineTo(-12,24);ctx.moveTo(6,14);ctx.lineTo(12,19)}
    else if(air){const tuck=clamp(Math.abs(p.vy)/750,0,1);ctx.moveTo(-6,14);ctx.lineTo(-10,22-tuck*4);ctx.moveTo(6,14);ctx.lineTo(11,22+tuck*2)}
    else{ctx.moveTo(-6,14);ctx.lineTo(-8+stride,27);ctx.moveTo(6,14);ctx.lineTo(8-stride,27)}ctx.stroke();
    ...
    if(tether){
      const a=(this.engine.world.anchors||[]).find(item=>item.id===p.sapline.anchorId),pos=this.engine.anchorPosition?.(a);let ang=-1.1;
      if(pos){const localX=(pos.x-p.x)*p.facing,localY=pos.y-p.y;ang=Math.atan2(localY,localX)}
      ctx.moveTo(6,-4);ctx.lineTo(6+Math.cos(ang)*19,-4+Math.sin(ang)*19);ctx.moveTo(-6,-3);ctx.lineTo(-11,7);
    }else if(attack){ctx.moveTo(6,-4);ctx.lineTo(15,-2);ctx.moveTo(-6,-3);ctx.lineTo(-10,5)}
    else{const armSwing=p.onGround?Math.sin(runPhase+Math.PI)*5:clamp(-p.vy/700,-1,1)*4;ctx.moveTo(6,-4);ctx.lineTo(10+armSwing,7);ctx.moveTo(-6,-3);ctx.lineTo(-10-armSwing,7)}ctx.stroke();
    ...
    ctx.restore();
    if(wall){ctx.fillStyle='rgba(177,214,156,.34)';ctx.fillRect(p.x+p.wallDir*17,p.y-15,2.5,30)}
  }
  ...
  drawBoss(b){
    ...
    // The Crown Girdler is a parasite clamped around the tree, not a large humanoid.
    ctx.strokeStyle='#0b100e';ctx.lineWidth=32;ctx.beginPath();ctx.arc(0,0,78,-2.75,.45);ctx.stroke();
    ctx.strokeStyle='#39423e';ctx.lineWidth=22;ctx.beginPath();ctx.arc(0,0,78,-2.75,.45);ctx.stroke();
    ctx.strokeStyle='#b7793c';ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,79,-2.7,.4);ctx.stroke();
    for(let i=0;i<3;i++){
      const a=-2.42+i*.95,intact=i<b.guard,px=Math.cos(a)*76,py=Math.sin(a)*76;
      ctx.save();ctx.translate(px,py);ctx.rotate(a+Math.PI/2);ctx.fillStyle=intact?'#2e3935':'#151b19';ctx.fillRect(-17,-12,34,24);ctx.strokeStyle=intact?COLORS.danger:'rgba(120,98,72,.35)';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-14,8);ctx.lineTo(-28,22);ctx.moveTo(14,8);ctx.lineTo(28,22);ctx.stroke();if(intact){ctx.fillStyle='#d68f47';ctx.beginPath();ctx.arc(0,-2,4+Math.sin(t*5+i)*.7,0,TAU);ctx.fill()}ctx.restore();
    }
    // Partial saw ring visibly accelerates with phase.
    ...
    const core=ctx.createRadialGradient(0,0,2,0,0,28);core.addColorStop(0,open?'rgba(224,255,198,.92)':'rgba(214,147,70,.72)');core.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=core;ctx.beginPath();ctx.arc(0,0,28,0,TAU);ctx.fill();
    ctx.fillStyle=open?COLORS.spirit:'#1a2521';ctx.beginPath();ctx.arc(0,0,10,0,TAU);ctx.fill();
    // Hydraulic housings make the boss read as machinery gripping both sides.
  ...
  drawWorld(){
    ...
    for(const rail of e.world.rails||[])this.drawBarkRail(rail);
