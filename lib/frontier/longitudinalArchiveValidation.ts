...
import {
  ...
  type LongitudinalRollupDimension,
...
const AMBIENT_KINDS = new Set<FrontierAmbientReactionKind>(FRONTIER_AMBIENT_REACTION_KINDS);
...
function tags(value: unknown): string[] | null {
  ...
  for (const entry of value) {
    ...
    if (!normalized || normalized.length > MAX_TAG || seen.has(normalized)) return null;
    seen.add(normalized);
...
function context(value: Record<string, unknown>): LongitudinalItemContext | null {
  ...
  if (!itemId || !lane || !LANE_IDS.has(lane) || !parsedTags || !sourceKind
    || !SOURCE_KINDS.has(sourceKind as FrontierSourceKind) || !format) return null;
  return {
    itemId,
...
function matchingDay(dayKey: string, timestamp: number): boolean {
  return dayKey === longitudinalDayKey(timestamp);
}
...
function parseReaction(value: unknown): LongitudinalReactionEpisode | null {
  ...
  if (!shared || !id || !sessionId || !exposureId || occurredAt === null || !dayKey || !matchingDay(dayKey, occurredAt)
    || !kind || !AMBIENT_KINDS.has(kind as FrontierAmbientReactionKind) || confidence === null || intensity === null
  ...
  if (value.reviewedAt !== undefined) {
    ...
    if (parsed === null || parsed < occurredAt) return null;
    reviewedAt = parsed;
  }
...
function parseInteraction(value: unknown): LongitudinalInteraction | null {
  ...
  const dayKey = day(value.dayKey);
  const kind = text(value.kind, 32);
  if (!shared || !id || !sessionId || at === null || !dayKey || !matchingDay(dayKey, at)
    || !kind || !INTERACTION_KINDS.has(kind as LongitudinalInteractionKind)) return null;
...
function parseCheckin(value: unknown): LongitudinalCheckin | null {
  ...
  if (!id || at === null || !dayKey || !matchingDay(dayKey, at) || mood === null || energy === null || focus === null) return null;
