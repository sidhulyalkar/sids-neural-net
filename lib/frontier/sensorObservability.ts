...
/**
 * Start a measurement episode at a monotonic timestamp such as performance.now().
 * Wall-clock time remains the authority for archive timestamps; this accumulator
 * exists only to integrate sampling availability without being distorted by a
...
/**
 * Integrate one inference callback into a target-attributed exposure.
 *
 * Only bounded time between actual callbacks is credited. A background-tab pause,
 * model stall, main-thread freeze, or suspended camera therefore cannot be mistaken
 * for observed face time. The interval is classified by the *current* callback's
 * face observability; no raw landmarks or image data enter this accumulator.
 */
...
export function sensorObservabilitySnapshot(
  value: SensorObservabilityAccumulator,
...
/**
 * The minimum privacy-safe payload needed to preserve measurement availability in
 * longitudinal storage. These are aggregate durations/counters only. They contain
 * no frames, landmarks, blendshape stream, biometric template, or identity signal.
 */
