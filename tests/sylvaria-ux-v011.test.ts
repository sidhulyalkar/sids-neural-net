...
test('Sylvaria keeps competitive UI restrained and hidden while v0.15 alpha ranking is intentionally unavailable',()=>{
  ...
  assert.match(html,/id="targetWrap" hidden/);assert.match(html,/id="rankPanel" class="rankPanel" hidden/);assert.match(entry15,/SylvariaRankedDisabledReason='v0\.15 Cutstep alpha · verifier migration required'/);assert.match(competitive,/if\(!current\|\|current\.practice\|\|current\.unrankedReason\|\|state\.mode!=='playing'\)/);assert.match(styles,/#targetWrap\[hidden\],#bossWrap\[hidden\]\{display:none!important\}/);
});
...
test('the dark forest alpha uses temporary visibility ecological refill and explicit threat coaching as player-facing rules',()=>{
  const html=read(`${runtimeRoot}/index.html`),entry15=read(`${runtimeRoot}/v015-entry.js`),forest=read(`${runtimeRoot}/v015/forest-world-v015.js`);assert.match(html,/Carve visibility/i);assert.match(entry15,/carve understory for micro-refill/i);assert.match(entry15,/clearing will grow shut behind you/i);assert.match(entry15,/Route first\. Cable and spore control/i);assert.match(entry15,/flank the heavy/i);assert.match(forest,/v015RegrowAt>0&&state\.roomTime>=f\.v015RegrowAt/);assert.match(forest,/drawBrushFog/);
});
