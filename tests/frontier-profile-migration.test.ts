...
test('profile migration backfills new explicit taste topics into old sparse profiles', () => {
  ...
  delete old.topicAffinity['neural foundation model'];
  delete old.topicAffinity['mechanistic interpretability'];
  delete old.topicAffinity['satellite imagery'];
  ...
  assert.ok((migrated.topicAffinity['neural foundation model'] ?? 0) > 0.3);
  assert.ok((migrated.topicAffinity['mechanistic interpretability'] ?? 0) > 0.3);
  assert.ok((migrated.topicAffinity['satellite imagery'] ?? 0) > 0.2);
});
...
test('profile migration never overwrites an explicit learned negative topic preference', () => {
  ...
  old.topicAffinity['mechanistic interpretability'] = -0.47;
  ...
  assert.equal(migrated.topicAffinity['mechanistic interpretability'], -0.47);
});
...
test('mature profiles keep learned lane affinities while still receiving missing topic vocabulary', () => {
  ...
  delete old.topicAffinity['eeg foundation model'];
  ...
  assert.ok((migrated.topicAffinity['eeg foundation model'] ?? 0) > 0.3);
});
