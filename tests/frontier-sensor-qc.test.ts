...
test('QC caps callback stalls instead of inventing continuously sampled time', () => {
  let trial = createSensorQcTrialAccumulator('neutral_reading', 0);
  trial = observeSensorQcSample(trial, {
    sampleAt: 0, wallAt: 0, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
  trial = observeSensorQcSample(trial, {
    sampleAt: 100, wallAt: 100, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
  trial = observeSensorQcSample(trial, {
    sampleAt: 5_100, wallAt: 5_100, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
...
test('QC separates face coverage, target attribution, and multi-card abstention', () => {
  let trial = createSensorQcTrialAccumulator('two_card_ambiguity', 0);
  trial = observeSensorQcSample(trial, {
    sampleAt: 0, wallAt: 0, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
  ...
  trial = observeSensorQcSample(trial, {
    sampleAt: 200, wallAt: 200, foreground: true, feedActive: true,
    faceObservable: false, targetAttributed: false, visibleCandidates: 2,
  });
...
test('hidden callbacks never receive sampled-time credit', () => {
  let trial = createSensorQcTrialAccumulator('background_tab', 0);
  trial = observeSensorQcSample(trial, {
    sampleAt: 0, wallAt: 0, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
  trial = observeSensorQcSample(trial, {
    sampleAt: 100, wallAt: 100, foreground: false, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
  trial = observeSensorQcSample(trial, {
    sampleAt: 200, wallAt: 200, foreground: true, feedActive: true,
...
test('out-of-order callbacks cannot create negative or duplicate duration', () => {
  let trial = createSensorQcTrialAccumulator('natural_browsing', 0);
  trial = observeSensorQcSample(trial, {
    sampleAt: 100, wallAt: 100, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
  trial = observeSensorQcSample(trial, {
    sampleAt: 90, wallAt: 110, foreground: true, feedActive: true,
    faceObservable: true, targetAttributed: true, visibleCandidates: 1,
  });
...
test('QC snapshot reports reviewed precision without treating unreviewed cues as failures', () => {
  const trial = createSensorQcTrialAccumulator('interesting_reading', 0);
  ...
  assert.equal(snapshot.cueRatePerMinute, 3);
});

test('QC export is aggregate-only and contains no content identifiers', () => {
  const trial = createSensorQcTrialAccumulator('neutral_reading', 0);
  trial.id = 'qc-trial-1';
  trial.endedAt = 1_000;
