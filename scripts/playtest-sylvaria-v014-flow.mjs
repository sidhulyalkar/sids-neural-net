...
const recoveredBlade=await waitForValue(base=>{const G=window.Sylvaria091,k=window.SylvariaKinetics.snapshot();if(G.state.stats.cuts<=base||k.arc?.direction!=='up')return false;return{cuts:G.state.stats.cuts,arc:k.arc,flow:window.SylvariaFlowCombat.snapshot()}},lateRecovery.cuts,800);
check(recoveredBlade.cuts===lateRecovery.cuts+1,'buffered recovery blade did not execute exactly once');
