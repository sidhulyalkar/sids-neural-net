...
// Learn Controls: it is non-ranked and forgiving until move + cut + reflect are demonstrated.
{
  const page=await browser.newPage({viewport:{width:1280,height:900}});let ticketRequests=0;
  ...
  assert(rescued.trees.every(tree=>tree.alive&&tree.hp===tree.maxHp),'practice failure did not restore protected trees');
...
// Mocked ranked deployment: one live target renders and a completed replay gets provisional then verified placement.
{
  ...
  const targetText=await page.locator('#targetState').textContent();
  assert(targetText?.includes('#3')&&targetText.includes('to go'),`nearest live leaderboard target was not concise/readable: ${targetText}`);
...
console.log('Sylvaria v0.11.1 competitive UX PASS: instant local start, graceful ranked fallback, repeat-safe pause/mute, keyboard-safe menu activation, forgiving action-driven coaching, practice isolation, one live score chase, provisional placement, replay posting, and verified rank resolution verified.');
