...
// Learn Controls: action-driven coaching, never a ranked ticket.
{
  const page=await browser.newPage({viewport:{width:1280,height:900}});let ticketRequests=0;
  ...
  await page.click('#explore');await page.waitForFunction(()=>window.SylvariaCoach?.snapshot().enabled===true&&window.__MOSSLIGHT_PLAYTEST__.snapshot().mode==='playing');
  ...
  await page.evaluate(()=>{const p=window.__MOSSLIGHT_PLAYTEST__;p.setRoom(1,2);p.clearCombatants();p.spawnCounterShot('right',72,{speed:180})});await page.waitForTimeout(40);await page.keyboard.press('ArrowRight');await page.waitForFunction(()=>window.SylvariaCoach.snapshot().stage===4,{timeout:1000});
...
{
  const page=await browser.newPage({viewport:{width:1280,height:900}});let posted=false,submitBody=null;
  ...
  await page.route('**/api/sylvaria/run-ticket',async route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ranked:true,configured:true,ticket:'mock.ticket.value.abcdefghijklmnopqrstuvwxyz',engineVersion:'0.11.1',engineHash:'a'.repeat(64),seed:110001,expiresAt:Date.now()+600000})}));
  ...
  assert(await page.locator('#rankRows li').first().textContent()?.then(text=>text?.includes('north')),'leaderboard target did not render');
  await page.click('#start');await page.waitForFunction(()=>window.SylvariaCompetitive.snapshot().current?.ticketReady===true);await page.waitForTimeout(550);await page.evaluate(()=>window.Sylvaria091.fn.endRun('test complete'));await page.waitForFunction(()=>document.getElementById('rankedState')?.textContent==='ranked replay ready');
...
console.log('Sylvaria v0.11.1 competitive UX PASS: instant local start, graceful ranked fallback, repeat-safe pause/mute, action-driven coaching, practice isolation, leaderboard targets, replay posting, and rank resolution verified.');
