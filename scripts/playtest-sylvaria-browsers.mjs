...
for (const { name, browserType, launchOptions } of engines) {
  ...
  try {
    ...
    await frame.waitForFunction(() => window.__SIDS_GAME_NETWORK_BRIDGE__ === true && window.__MOSSLIGHT_PLAYTEST__?.version === '0.8.0' && window.SylvariaVisualSystem?.version === '0.8.0');
    ...
    if (identity.title !== 'Sylvaria' || identity.version !== '0.8.0') throw new Error(`identity mismatch ${JSON.stringify(identity)}`);
    if (identity.roomCount !== 10) throw new Error(`authored room count mismatch ${identity.roomCount}`);
    if (!identity.visual?.playfieldAspectSafe || !identity.visual?.immersiveControl) throw new Error(`visual contract incomplete ${JSON.stringify(identity.visual)}`);
    ...
    if ((after.player?.x ?? 0) <= (before.player?.x ?? 0) + 35) throw new Error(`step-dash input failed ${before.player?.x} -> ${after.player?.x}`);
    if (after.stats.cuts <= before.stats.cuts) throw new Error('ArrowUp did not register a machete cut');
    ...
    await page.screenshot({ path: path.join(outputDir, `${name}-countercut.png`), fullPage: true });
    report.push({ name, ok: true, identity, titleStats, playStats, movement: [before.player?.x, after.player?.x], cuts: after.stats.cuts - before.stats.cuts, fps: after.fps });
  } catch (error) {
    failed = true;
...
console.log(`Sylvaria Countercut browser matrix PASS: ${report.map((entry) => entry.name).join(', ')}.`);
