...
const html=read(`${root}/index.html`),model=read(`${root}/v091/model.js`),world=read(`${root}/v091/world.js`),boot=read(`${root}/v091/boot.js`),rooms=read(`${root}/v011/rooms-v011.js`),competitive=read(`${root}/v011/competitive-v011.js`),entry13=read(`${root}/v013-entry.js`),replay13=read(`${root}/v013/replay-v013.js`),entry14=read(`${root}/v014-entry.js`),boss14=read(`${root}/v014/boss-flow-v014.js`),threat14=read(`${root}/v014/threat-manager-v014.js`),enemy14=read(`${root}/v014/enemy-flow-v014.js`),space14=read(`${root}/v014/presentation-space-v014.js`),entry15=read(`${root}/v015-entry.js`),cut15=read(`${root}/v015/cutstep-v015.js`),forest15=read(`${root}/v015/forest-world-v015.js`),presentation15=read(`${root}/v015/cutstep-presentation-v015.js`),arcade=read('src/data/arcadeGames.ts'),serverReplay=read('src/lib/sylvaria/replay.ts');

expect(has(html,'<title>Sylvaria · Cutstep Forest</title>','v0.15 · CUTSTEP CLEARING','01 / forest','Whispering Pine Verge','WASD moves your body','Arrow keys or mouse aim independently','SPACE or click instantly Cutsteps','sylvaria-forest-v015.css'),'v0.15 forest shell incomplete');
...
expect(has(entry15,"await import('./v014-entry.js')",'v015/cutstep-v015.js','v015/forest-world-v015.js','v015/cutstep-presentation-v015.js',"window.SylvariaRankedDisabledReason='v0.15 Cutstep prototype · verifier migration required'",'ranked:false'),'v0.15 entry or ranking boundary incomplete');
...
expect(has(cut15,"CUTSTEP_VERSION='0.15.0'",'maxSegments:3','distance:86','thrustDistance:104','ticks:9','passiveRefillPerSecond:.55','brushRefillPerBlade:.018','counterRefill:.42','killRefill:.48',"kind:'thrust'","kind:'crosscut'","kind:'reversal'",'F.beginDashCharge=()=>false;F.releaseDashCharge=()=>false','launchCutstep(currentAim())'),'Cutstep core contract incomplete');
expect(has(cut15,"const active=p.dash?.v015Cutstep?p.dash:null",'if(active)state.heldMoves=new Set()','finally{if(active)state.heldMoves=held}'),'WASD can still bend an aimed Cutstep');
expect(has(cut15,'dash.v015HitIds.has(s)','dash.v015HitIds.add(s)','reflectShot(s,dash.dir,p)','F.damageEnemy?.','f.cut=true;f.v015RegrowAt='),'Cutstep line does not multiply across projectiles enemies and brush');
...
expect(has(forest15,'brush:11','brush:12','brush:13','20+Math.floor(r/3.5)','v015RegrowAt>0&&state.roomTime>=f.v015RegrowAt','function drawPine','function drawOak','function drawBurned','function drawBrushFog','Clearcut Hand','Nailgun Logger','Feller Mech','Root Mulcher'),'dark forest density/regrowth/character language incomplete');
...
expect(has(presentation15,"overlay.id='cutstepCanvas'",'drawHistory','drawActive','drawAim','drawSegments','prospectiveTechnique'),'Cutstep aim/path presentation incomplete');
expect(has(space14,"['forestCanvas','kineticCanvas','flowCanvas','cutstepCanvas']",'ctx?.setTransform(scale,0,0,scale,0,0)','logicalToClient','clientToLogical'),'v0.15 overlays are not DPR-safe');
...
const authoritativeHash=crypto.createHash('sha256').update([model,world,rooms,cut15,forest15,boss14,threat14].join('\n')).digest('hex');
...
console.log(`Sylvaria v0.15 validator PASS · instant independent-aim Cutstep · three-segment geometry · dense regrowing dark forest · preserved deterministic combat substrate · sha256 ${authoritativeHash}`);
