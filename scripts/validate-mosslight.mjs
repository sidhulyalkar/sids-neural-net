...
// Production must boot the v0.11.1 adapter, while the protected lower-level simulation remains unchanged.
expect(has(html, '<title>Sylvaria</title>', 'sylvaria-minimal-v011.css', 'id="fieldState"', '30 fixed arenas', 'start run', 'practice'), 'v0.11.1 minimal shell is incomplete');
...
expect(has(entry, "import './v091/model.js'", "import('./v011/rooms-v011.js')", "import('./v091/boot.js')", "import('./v091/synergy-v010.js')", "import('./v011/presentation-v011.js')", "import('./v011/replay-v011.js')", "VERSION='0.11.1'", 'roomCount=rooms.length', 'expandedArenas:true', 'minimalPresentation:true', 'deterministicReplay:true'), 'v0.11.1 entry/load-order adapter incomplete');
...
// Inherited v0.10 interaction layer remains internal and must not rewrite protected numbers.
...
// v0.11.1 turns depth 1..30 into fixed learnable arenas, then returns to deterministic endless generation.
...
// Minimal presentation removes vocabulary, not mechanics.
...
expect(has(html, 'Move with WASD', 'Cut with the arrow keys', 'Simple inputs. Difficult rooms.', 'health'), 'minimal control copy is incomplete');
expect(has(minimalStyles, '#roomCard', '#runRail', '.telemetryHidden', '.menuPanel', 'box-shadow:none'), 'minimal UI stylesheet missing hierarchy reductions');
...
// Ranked replay identity and source hash must include the authored arena rules.
...
for (const [name, source] of Object.entries({model:files.model,world:files.world,movement:files.movement,'battle-core':files['battle-core'],render:files.render,'synergy-v010':synergy,'rooms-v011':rooms,'presentation-v011':presentation,'replay-v011':replay})) {
...
// v011-entry uses top-level await by design; production build/browser gates validate module semantics/load order.
...
console.log('Sylvaria v0.11.1 PASS: protected 120 Hz reflection/queue mechanics remain unchanged; 30 fixed arenas, minimal player-facing vocabulary, bounded replay capture, authored-room engine hashing, CPU-bounded Node verification, and inherited terrain/poison interactions are wired without rewriting the qualified core.');