...
expect(has(html,'<title>Sylvaria · Kinetic Pond</title>','v0.13 · kinetic arc combat','Glide. Charge. Sweep. Return fire.','glide north','tongue arc up','hold / release dash','id="rankPanel"','id="rankedSubmit"'),'v0.13 kinetic pond shell incomplete');
...
// Continuous deterministic locomotion and a charged velocity burst replace scripted dash-step travel.
expect(has(kinetics,"KINETIC_VERSION='0.13.0'",'moveSpeed:238','acceleration:1880','braking:12.5','turnGrip:15.5','dashChargeTime:.68','dashMinSpeed:520','dashMaxSpeed:960','function heldVector','function beginDashCharge','function releaseDashCharge','function updateMovement(dt)','p.vx=q(','p.vy=q(','function currentAt'),'v0.13 continuous locomotion or charge dash incomplete');
...
expect(has(kinetics,'arcDegrees:156','arcWindup:.058','arcActive:.146','arcRecovery:.112','arcReach:94','reflectSpeed:920','perfectReflectSpeed:1120',"phase:'windup'","s.phase='active'","s.phase='recovery'",'function arcSweepContains','function fullArcContains','function counterShotArc','SWEET SPOT'),'kinetic tongue arc or reflect timing incomplete');
expect(has(kinetics,'tangent=s.sweepDir>0','Math.abs(s.activeProgress-.5)<=s.perfectWindow'),'reflection is not based on the moving arc and mid-swing timing');
...
expect(has(kineticPresentation,"id='kineticCanvas'",'function drawArcAttack',"s.phase==='windup'", "s.phase==='active'",'quadraticCurveTo','function drawDash','function drawKineticEnemies',"state.slashes.filter(s=>s.kind!=='arc')"),'v0.13 arc/dash presentation incomplete');
...
expect(has(coach13,"VERSION='0.13.0'",'WASD · glide freely','hold SPACE · charge · release to dash','arrow key · sweep the tongue','mid-swing · reflect'),'v0.13 onboarding does not teach the new grammar');
...
expect(has(arcade,"version: 'v0.13.0'",'GLIDE · SWEEP · RETURN FIRE.','continuous pond movement','charged dash','reactive ai'),'Game Network metadata is not on v0.13');
...
console.log(`Sylvaria v0.13 kinetic combat validator PASS · 120 Hz deterministic engine · authoritative sha256 ${authoritativeHash}`);