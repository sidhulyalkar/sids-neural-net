...
expect(has(entry12,"import'./v011-entry.js'","PRESENTATION='0.12.0',ENGINE='0.11.1'",'createPondRenderer','F.render=','F.updateHud=','presentationVersion:PRESENTATION','frogPond:true','tongueAttack:true','insectEnemies:true','pondEnvironment:true'),'v0.12 adapter incomplete');
...
expect(has(atlas,'function drawFrog','function buildHeight','getImageData',"filter='blur(1.25px)'"),'height-atlas generation incomplete');
...
expect(has(pondCss,'#pondCanvas','html.pond-webgl #c{opacity:0}','pointer-events:none'),'WebGL overlay/fallback CSS incomplete');
...
expect(has(profiler,"presentationVersion: '0.12.0'","engineVersion: '0.11.1'",'v012/art-atlas-v012.js','v012/webgl-pond-v012.js'),'size profiler does not measure the shipping v0.12 renderer');
...
const presentationHash=crypto.createHash('sha256').update([entry12,pond,atlas,pondCss,html].join('\n')).digest('hex');
