...
// Begin comfortably inside the 100 ms buffer window. Keep Space down/up as separate
// browser events so this lab proves both the held reservation and released reservation
// before the cooldown reaches zero, rather than racing a combined Playwright press.
const setup=await page.evaluate(()=>{const play=window.__MOSSLIGHT_PLAYTEST__,G=window.Sylvaria091,p=G.state.player;play.clearCombatants();play.labClearGeometry();play.setPlayerPosition(360,340);G.state.heldMoves.clear();G.state.heldOrder=[];p.dash=null;p.dashCharging=false;p.dashCharge=0;p.dashCooldown=10/120;p.dashBuffer=0;p.dashBufferHeld=false;p.dashBufferReleased=false;p.vx=0;p.vy=0;p.lastDashAccuracy=null;return{dashes:G.state.stats.dashes,cooldown:p.dashCooldown,bufferWindow:window.SylvariaKinetics.config.dashBuffer}});
await page.keyboard.down('d');await page.keyboard.down('Space');
const queuedHeld=await page.evaluate(()=>{const G=window.Sylvaria091,p=G.state.player;return{flow:window.SylvariaFlowCombat.snapshot(),kinetics:window.SylvariaKinetics.snapshot(),dashes:G.state.stats.dashes,cooldown:p.dashCooldown,buffer:p.dashBuffer,bufferHeld:p.dashBufferHeld,bufferReleased:p.dashBufferReleased,dashing:Boolean(p.dash)}});
check(queuedHeld.buffer>0&&queuedHeld.bufferHeld===true&&queuedHeld.bufferReleased===false,`Space-down did not reserve dash during recovery: ${JSON.stringify(queuedHeld)}`);
check(queuedHeld.dashes===setup.dashes&&!queuedHeld.dashing,'held buffered Space launched before cooldown became legal');
await page.keyboard.up('Space');
const queuedReleased=await page.evaluate(()=>{const G=window.Sylvaria091,p=G.state.player;return{dashes:G.state.stats.dashes,cooldown:p.dashCooldown,buffer:p.dashBuffer,bufferHeld:p.dashBufferHeld,bufferReleased:p.dashBufferReleased,dashing:Boolean(p.dash)}});
check(queuedReleased.buffer>0&&queuedReleased.bufferHeld===false&&queuedReleased.bufferReleased===true,`Space-up did not convert held reservation into released dash buffer: ${JSON.stringify(queuedReleased)}`);
check(queuedReleased.dashes===setup.dashes&&!queuedReleased.dashing,'released buffer launched before cooldown became legal');
...
const report={setup,queuedHeld,queuedReleased,launched,ended,failures};fs.writeFileSync(path.join(outputDir,'buffer-accuracy-report.json'),JSON.stringify(report,null,2));await browser.close();
...
console.log(`Sylvaria v0.14 buffered-dash accuracy PASS · held/released reservation survived recovery · one queued dash traveled ${ended.accuracy.pathTravel.toFixed(3)} px against ${ended.accuracy.distanceTarget.toFixed(3)} px solved target.`);