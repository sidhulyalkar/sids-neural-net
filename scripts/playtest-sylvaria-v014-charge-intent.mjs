...
check(beforeRelease.flow.dashChargeVector.x<-.65&&beforeRelease.flow.dashChargeVector.y<-.65,`sequential key-up collapsed remembered diagonal during release grace: ${JSON.stringify(beforeRelease)}`);
...
// Deliberate retarget: releasing only A while continuing to hold W must become north after the fixed grace expires.
await resetChargeLab();
await page.keyboard.down('a');await page.keyboard.down('w');await page.keyboard.down('Space');
await page.waitForFunction(()=>{const p=window.Sylvaria091.state.player;return p.dashCharging&&p.dashCharge>.22&&p.dashChargeVector.x<-.65&&p.dashChargeVector.y<-.65},{timeout:900});
await page.keyboard.up('a');
await page.waitForFunction(()=>{const f=window.SylvariaFlowCombat.snapshot();return f.chargeReleaseGrace===0&&Math.abs(f.dashChargeVector.x)<.05&&f.dashChargeVector.y<-.95},{timeout:700});
const retargeted=await page.evaluate(()=>({flow:window.SylvariaFlowCombat.snapshot(),held:[...window.Sylvaria091.state.heldMoves]}));
await page.keyboard.up('Space');
...
check(retargeted.held.length===1&&retargeted.held[0]==='w',`deliberate retarget did not leave only W held: ${JSON.stringify(retargeted)}`);
check(Math.abs(retargetLaunch.dir.x)<.03&&retargetLaunch.dir.y<-.97,`held W did not retarget charged dash after grace: ${JSON.stringify({retargeted,retargetLaunch})}`);

const report={charged,beforeRelease,launched,retargeted,retargetLaunch,failures};fs.writeFileSync(path.join(outputDir,'charge-intent-report.json'),JSON.stringify(report,null,2));
await browser.close();
...
console.log(`Sylvaria v0.14 charge-intent PASS · chord release preserved northwest (${launched.dir.x.toFixed(3)}, ${launched.dir.y.toFixed(3)}) · deliberate post-grace retarget launched north (${retargetLaunch.dir.x.toFixed(3)}, ${retargetLaunch.dir.y.toFixed(3)}).`);