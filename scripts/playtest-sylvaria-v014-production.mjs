...
// without leaking HP, then the explicit core opening amplifies blade damage while live pressure stays intact.
const boss=await page.evaluate(()=>{const play=window.__MOSSLIGHT_PLAYTEST__,G=window.Sylvaria091;play.setRoom(29,30);play.labClearGeometry();play.setPlayerPosition(300,330);for(const e of G.state.enemies)e.dead=true;const b=G.state.boss;b.phase=3;b.v014LastPhase=3;b.v014GuardMax=5;b.v014Guard=5;b.v014PunishTimer=0;b.state='telegraph';b.telegraph=.5;b.clock=.5;const shotsBefore=G.state.shots.length,hpBeforeGuard=b.hp,guard=[],hpDuringGuard=[];for(let i=0;i<5;i++){b.state='telegraph';b.telegraph=.5;G.fn.damageBoss(1,null,{arc:true,melee:true,attack:{dashCancelled:true}});guard.push(b.v014Guard);hpDuringGuard.push(b.hp)}G.fn.render();const opened=window.SylvariaBossFlow.snapshot(),presentation=window.SylvariaFlowPresentation.snapshot(),hp=b.hp;G.fn.damageBoss(1,{x:1,y:0},{arc:true,melee:true});const bladeDamage=hp-b.hp;return{guard,hpBeforeGuard,hpDuringGuard,opened,presentation,bladeDamage,shotsBefore,shotsAfter:G.state.shots.length,state:b.state,telegraph:b.telegraph}});
...
check(boss.hpDuringGuard.every(hp=>Math.abs(hp-boss.hpBeforeGuard)<1e-6),`closed guard leaked HP damage: ${JSON.stringify({before:boss.hpBeforeGuard,during:boss.hpDuringGuard})}`);
