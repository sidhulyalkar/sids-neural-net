...
export function GestureController() {
  ...
  useEffect(() => {
    if (!enabled || status !== 'running' || !cursor) {
      scheduleUi(() => setTargetLabel(null));
      return;
    }
    if (paletteOpen) {
    ...
    const nextLabel = label ? label.replace(/\s+/g, ' ').slice(0, 48) : null;
    scheduleUi(() => setTargetLabel(nextLabel));
  ...
  useEffect(() => {
    ...
    switch (action.type) {
      ...
      case 'page_down': {
        ...
        window.scrollBy({ top: window.innerHeight * 0.8, behavior: reducedMotion ? 'auto' : 'smooth' });
        break;
      }
      case 'prank': {
    ...
    if (feedbackTimeoutRef.current !== null) window.clearTimeout(feedbackTimeoutRef.current);
    feedbackTimeoutRef.current = window.setTimeout(() => setFeedback(null), action.type === 'prank' ? 3200 : 1200);
  ...
  return (
    <>
      {cursor && (
        <div aria-hidden="true" className="pointer-events-none fixed z-[90] -translate-x-1/2 -translate-y-1/2 transition-[left,top] duration-75" style={{ left: `${cursor.x * 100}%`, top: `${cursor.y * 100}%` }}>
          <div className={`grid h-8 w-8 place-items-center rounded-full border-2 backdrop-blur-sm transition-transform ${cursor.pinching ? 'scale-75 border-amber bg-amber/25 shadow-glow-amber' : targetLabel ? 'border-green bg-green/15 shadow-glow-green' : 'border-violet bg-violet/10 shadow-glow-violet'}`}>
            <span className="h-1.5 w-1.5 rounded-full bg-white" />
          </div>
          {targetLabel && <span className="absolute left-5 top-7 w-max max-w-48 rounded bg-bg-deep/90 px-2 py-1 font-mono text-[9px] text-text-secondary">{targetLabel}</span>}
        </div>
      )}

      <div className="pointer-events-none fixed left-1/2 top-5 z-[90] -translate-x-1/2 rounded-full border border-violet/25 bg-bg-panel/85 px-3 py-1.5 font-mono text-[10px] text-text-secondary backdrop-blur" aria-live="polite">
        <span className="text-violet">{feedback || pose.replaceAll('_', ' ')}</span><span className="mx-2 text-white/20">·</span>{fps} fps
      </div>

      {pranked && <div className="pointer-events-none fixed inset-x-4 top-[42%] z-[100] text-center" role="status" aria-live="assertive"><span className="inline-block rounded-2xl border border-amber/45 bg-bg-deep/95 px-6 py-4 font-mono text-sm font-semibold text-amber shadow-glow-amber backdrop-blur">Okay, you got me. The site looked. 👀</span></div>}

      {paletteOpen && (
        <div className="fixed inset-0 z-[80] flex items-start justify-center bg-bg-deep/65 px-4 pt-[12vh] backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Gesture navigation" data-gesture-palette onMouseDown={(event) => { if (event.target === event.currentTarget) setPaletteOpen(false); }}>
          <Command className="w-full max-w-xl overflow-hidden rounded-2xl border border-violet/25 bg-bg-panel shadow-glow-violet">
            <div className="flex items-center gap-2 border-b border-white/10 px-4">
              ...
              <button type="button" onClick={() => setPaletteOpen(false)} aria-label="Close navigation" className="rounded p-1 text-text-muted hover:text-text-primary"><X className="h-4 w-4" /></button>
            </div>
            <Command.List className="max-h-[55vh] overflow-y-auto p-2">
              ...
              <Command.Group heading="Navigate" className="text-xs text-text-muted">
                {siteNavItems.map((item) => (
                  <Command.Item key={item.href} value={`${item.label} ${item.description}`} onSelect={() => { router.push(item.href); setPaletteOpen(false); }} className="group flex cursor-pointer items-start gap-3 rounded-lg px-3 py-2.5 text-text-secondary outline-none data-[selected=true]:bg-violet/15 data-[selected=true]:text-text-primary">
                    <Hand className="mt-0.5 h-3.5 w-3.5 shrink-0 text-violet/70" /><span><span className="block text-sm font-medium">{item.label}</span><span className="block text-[11px] text-text-muted">{item.description}</span></span>
                  </Command.Item>
                ))}
              </Command.Group>
            </Command.List>
            <div className="border-t border-white/10 px-4 py-2 font-mono text-[9px] text-text-muted">Point + pinch to choose · closed fist to cancel</div>
          </Command>
        </div>
      )}
    </>
