...
type Ripple = { id: number; worldId: string; x: number; y: number };
type PointerState = { worldId: string; x: number; y: number };
...
function ActivityOverlay({ activity }: { activity: PersonaActivity }) {
  ...
  if (activity === 'build-cairn') return <g transform="translate(760 570)">{[[0,0,42,20],[-2,-27,32,16],[2,-50,22,13]].map(([x,y,rx,ry],index)=><ellipse key={index} cx={x} cy={y} rx={rx} ry={ry} fill={index===0?'#686f6e':index===1?'#7a807e':'#909694'} />)}</g>;
  if (activity === 'garden') return <g transform="translate(735 580)"><path d="M -85 0 L 85 0 L 67 48 L -72 48 Z" fill="#5c4838" opacity="0.9" />{[-55,-18,20,57].map((x,index)=><g key={x} transform={`translate(${x} 0)`}><path d="M 0 0 Q 0 -28 0 -48" stroke="#6e915e" strokeWidth="5" /><ellipse cx={index%2?9:-9} cy="-36" rx="12" ry="5" fill="#7da36c" transform={`rotate(${index%2?-25:25} ${index%2?9:-9} -36)`} /></g>)}</g>;
  if (activity === 'collect') return <g transform="translate(760 568)"><path d="M -48 5 Q 0 30 48 5 L 38 46 L -38 46 Z" fill="#8b6747" /><path d="M -30 8 Q 0 -35 30 8" fill="none" stroke="#a9855d" strokeWidth="7" />{[-26,0,26].map((x,index)=><circle key={x} cx={x} cy={-4-index*5} r="7" fill={index===0?'#b9e9d5':index===1?'#d7c2f2':'#efd48c'}><animate attributeName="opacity" values="0.4;1;0.4" dur={`${1.5+index*0.3}s`} repeatCount="indefinite" /></circle>)}</g>;
  if (activity === 'stargaze') return <g transform="translate(760 583)"><rect x="-63" y="-12" width="126" height="24" rx="10" fill="#6f647e" opacity="0.82" transform="rotate(-5)" /><path d="M 46 -16 L 72 -54" stroke="#b2bbc2" strokeWidth="5" /><circle cx="76" cy="-61" r="9" fill="#cbd5db" /></g>;
  if (activity === 'skip-stones') return <g transform="translate(790 545)">{[0,1,2].map((index)=><g key={index}><ellipse cx={index*42} cy={index*9} rx="18" ry="5" fill="none" stroke="rgba(225,248,250,0.5)" strokeWidth="2"><animate attributeName="rx" values="4;22;4" dur="1.7s" begin={`${index*0.25}s`} repeatCount="indefinite" /><animate attributeName="opacity" values="0.7;0;0" dur="1.7s" begin={`${index*0.25}s`} repeatCount="indefinite" /></ellipse></g>)}</g>;
  if (activity === 'snow-angel') return <g transform="translate(600 610)" opacity="0.34"><ellipse cx="0" cy="0" rx="42" ry="72" fill="none" stroke="#f5fdff" strokeWidth="9" /><path d="M -32 -10 Q -100 -70 -105 -6 M 32 -10 Q 100 -70 105 -6 M -20 57 L -62 115 M 20 57 L 62 115" fill="none" stroke="#f5fdff" strokeWidth="10" strokeLinecap="round" /></g>;
  return null;
}
...
export function NatureWorld2D({ snapshot, mood, accent, worldId, activity }: Props) {
  ...
  const [pointerState, setPointerState] = useState<PointerState>({ worldId, x: 0, y: 0 });
  const [ripples, setRipples] = useState<Ripple[]>([]);
  const reducedMotion = useMediaQuery('(prefers-reduced-motion: reduce)');
  const pointer = pointerState.worldId === worldId ? pointerState : { worldId, x: 0, y: 0 };
  const visibleRipples = ripples.filter((ripple) => ripple.worldId === worldId);
  ...
  const updatePointer = (clientX: number, clientY: number, element: HTMLDivElement) => {
    ...
    setPointerState({ worldId, x: Math.max(-1, Math.min(1, x)), y: Math.max(-1, Math.min(1, y)) });
  };

  const addRipple = (clientX: number, clientY: number, element: HTMLDivElement) => {
    if (!interactiveWater) return;
    const rect = element.getBoundingClientRect();
    const ripple = { id: Date.now(), worldId, x: ((clientX - rect.left) / rect.width) * 100, y: ((clientY - rect.top) / rect.height) * 100 };
    setRipples((current) => [...current.filter((item) => item.worldId === worldId).slice(-3), ripple]);
  ...
  return (
    <div
      ...
      onPointerLeave={() => setPointerState({ worldId, x: 0, y: 0 })}
    ...
    >
      ...
      {visibleRipples.map((ripple) => <span key={ripple.id} aria-hidden className="pointer-events-none absolute h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border border-cyan/60" style={{ left: `${ripple.x}%`, top: `${ripple.y}%`, animation: 'nature-ripple 1.25s ease-out forwards' }} />)}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/25 to-transparent px-4 pb-4 pt-20 sm:px-5 sm:pb-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="max-w-xl"><p className="font-mono text-[0.6rem] uppercase tracking-[0.16em] text-white/48">world {String(world.index).padStart(3, '0')} · {world.scene.collectionLabel}</p><p className="mt-1 text-sm font-medium text-white/92">{world.icon} {world.name}</p><p className="mt-1 hidden max-w-lg text-[0.65rem] leading-5 text-white/52 sm:block">{world.description}</p></div>
          <div className="text-right"><p className="font-mono text-[0.58rem] uppercase tracking-[0.12em] text-white/38">{world.scene.atmosphere} · {world.scene.density.toFixed(2)} density</p>{interactiveWater && <p className="mt-1 text-[0.6rem] text-cyan-100/55">tap the water for a ripple</p>}</div>
        </div>
      </div>

      <style jsx global>{`@keyframes nature-ripple {0% { width: 1rem; height: 0.35rem; opacity: 0.75; transform: translate(-50%, -50%) scale(0.5); }70% { opacity: 0.25; }100% { width: 9rem; height: 2.3rem; opacity: 0; transform: translate(-50%, -50%) scale(1.1); }}`}</style>
