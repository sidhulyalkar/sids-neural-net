...
export function NatureWorldAtlas({ currentWorldId, progress, onChooseWorld, onToggleFavorite }: AtlasProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return NATURE_WORLDS.filter((world) => {
      ...
      return [
        world.name, world.description, world.theme, world.terrain, world.collection, world.scene.collectionLabel,
        world.scene.atmosphere, world.scene.depth, world.scene.visualThesis, ...world.scene.renderCues, ...world.features, ...world.wildlife,
    ...
    <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
        <label className="flex min-h-10 items-center gap-2 rounded-lg border border-white/10 bg-black/10 px-3 text-xs text-text-secondary focus-within:border-cyan/30">
          ...
          <input value={query} onChange={(event) => { setQuery(event.target.value); setPage(0); }} placeholder="search aurora, sloth, rain, lavender, crystal, river, shell..." className="w-full bg-transparent text-text-primary outline-none placeholder:text-text-secondary/35" />
        </label>
        <button type="button" onClick={() => { setFavoritesOnly((value) => !value); setPage(0); }} className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border px-3 text-xs transition ${favoritesOnly ? 'border-rose-300/30 bg-rose-300/[0.07] text-rose-200' : 'border-white/10 text-text-secondary hover:border-white/20 hover:text-text-primary'}`}>
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
        {NATURE_COLLECTIONS.map((item) => (
          <button key={item.id} type="button" onClick={() => { setCollection(item.id); setPage(0); }} className={`shrink-0 rounded-full border px-3 py-1.5 text-[0.68rem] transition ${collection === item.id ? 'border-cyan/35 bg-cyan/[0.08] text-cyan' : 'border-white/10 text-text-secondary hover:border-white/20'}`}>
      ...
      <div className="mt-4">
        {visible.length === 0 ? (
          <div className="rounded-xl border border-dashed border-white/10 p-8 text-center text-xs text-text-secondary/50">No tiny world matches that search. The creature has checked under several extremely legitimate rocks.</div>
        ) : (
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {visible.map((world) => {
              ...
              return (
                <article key={world.id} className={`group relative overflow-hidden rounded-xl border transition ${selected ? 'border-cyan/45 bg-cyan/[0.06]' : 'border-white/10 bg-black/10 hover:border-white/20 hover:bg-white/[0.025]'}`}>
                  <button type="button" onClick={() => onChooseWorld(world.id)} className="w-full p-3.5 text-left">
                    <NatureWorldThumbnail2D world={world} />
                    <div className="flex items-start gap-2"><span className="text-base" aria-hidden>{world.icon}</span><div className="min-w-0"><p className="text-xs leading-4 text-text-primary">{String(world.index).padStart(3, '0')} · {world.name}</p><p className="mt-1 line-clamp-2 text-[0.61rem] leading-4 text-text-secondary/48">{world.description}</p></div></div>
                    <div className="mt-3 flex flex-wrap items-center gap-1.5 font-mono text-[0.54rem] uppercase tracking-[0.07em] text-text-secondary/38"><span>{world.scene.depth}</span><span>·</span><span>{world.scene.atmosphere}</span><span>·</span><span>{progress.visits[world.id] ?? 0} visits</span></div>
                  </button>
                  <button type="button" onClick={() => onToggleFavorite(world.id)} aria-label={`${favorite ? 'Remove' : 'Add'} ${world.name} ${favorite ? 'from' : 'to'} favorites`} className={`absolute right-2.5 top-2.5 flex h-8 w-8 items-center justify-center rounded-full border backdrop-blur transition ${favorite ? 'border-rose-200/30 bg-rose-300/15 text-rose-200' : 'border-white/10 bg-black/20 text-white/45 opacity-70 hover:opacity-100'}`}><Heart className={`h-3.5 w-3.5 ${favorite ? 'fill-current' : ''}`} /></button>
                  {discovered && <div className="pointer-events-none absolute left-2.5 top-2.5 flex h-6 w-6 items-center justify-center rounded-full border border-emerald-200/20 bg-emerald-300/10 text-emerald-200" title="discovered"><Check className="h-3 w-3" /></div>}
                </article>
              );
            })}
          </div>
      ...
      <div className="mt-4 flex items-start gap-2 rounded-lg border border-white/10 bg-black/10 p-3 text-[0.64rem] leading-5 text-text-secondary/50"><Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0 text-cyan/60" />The atlas is intentionally soft progression. Visiting records discovery, favoriting is an explicit preference signal, and recommendation-selected wandering never trains itself. Search and collection filters are presentation only.</div>
    </section>
  );
}
