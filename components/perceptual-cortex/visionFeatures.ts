...
export type FaceFeatures = { active: boolean; yaw: number; pitch: number; roll: number; activity: number; stillness: number };
...
export function deriveFaceFeatures(matrix: number[] | undefined, blendshapes: number[], previousActivity = 0): FaceFeatures {
  const activity = blendshapes.length ? Math.min(1, blendshapes.reduce((sum, value) => sum + value, 0) / blendshapes.length * 3) : 0;
  ...
  return { active: !!matrix || blendshapes.length > 0, yaw, pitch, roll, activity, stillness: 1 - Math.min(1, Math.abs(activity - previousActivity) * 5) };
}
