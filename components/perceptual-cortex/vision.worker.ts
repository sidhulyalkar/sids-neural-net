...
type BlendshapeCategory = { score: number; categoryName?: string; displayName?: string };
type Detector = {
...
let hand: Detector | null;
let face: Detector | null;
let previousPalms: Landmark[] = [];
let previousTime = 0;
...
self.onmessage = async (event: MessageEvent) => {
  ...
  if (event.data.type === 'frame') {
    ...
    try {
      ...
      const palms = landmarks.map((points) => [0, 5, 9, 13, 17]
        .map((id) => points[id])
        .reduce((sum, point) => ({
          x: sum.x + point.x / 5,
          y: sum.y + point.y / 5,
      ...
      for (const category of faceResult.faceBlendshapes?.[0]?.categories ?? []) {
        const name = category.categoryName || category.displayName;
        if (name) blendshapes[name] = category.score;
      }
      const faceFeatures = deriveFaceFeatures(matrix, blendshapes, previousActivity);
