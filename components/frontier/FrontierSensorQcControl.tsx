...
export function FrontierSensorQcControl({ feedActive }: { feedActive: boolean }) {
  ...
  useEffect(() => {
    ...
    const sample = (event: Event) => {
      ...
      const activeFeed = feedActiveRef.current;
      const frame = foreground && activeFeed
        ? measureSensorQcTargetFrame(useFrontierStore.getState().history)
        : { visibleCandidates: 0, targetAttributed: false };
      sensorQcRecordSample({
        sampleAt: detail.sampleAt,
        wallAt: detail.wallAt,
        foreground,
        feedActive: activeFeed,
