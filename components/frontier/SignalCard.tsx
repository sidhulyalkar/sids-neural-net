...
function SignalArt({ item }: { item: FrontierItem }) {
  ...
  return (
    <div className={styles.signalArt} aria-hidden="true">
      <div className={styles.signalGlyph}>{lane.glyph}</div>
    </div>
  );
}
  ...
  return (
    <div className={styles.media}>
      {youtube && variant === 'feature' ? (
        ...
        // External discovery media can originate from many sources, so next/image's static host allowlist is intentionally not used here.
...
export function SignalCard({
  ...
  const timeLabel = Number.isNaN(published.getTime())
    ? 'recent'
    : new Intl.RelativeTimeFormat('en', { numeric: 'auto' }).format(
        Math.round((published.getTime() - Date.now()) / 86_400_000),
        'day'
      );
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.scoreRail} aria-label="Signal scores">
          {[
            ...
            <div className={styles.scoreCell} key={String(label)}>
              <div className={styles.scoreLabel}><span>{label}</span><span>{score(Number(value))}</span></div>
              <div className={styles.scoreTrack}><div className={styles.scoreFill} style={{ width: `${score(Number(value))}%` }} /></div>
            </div>
          ))}
        </div>
