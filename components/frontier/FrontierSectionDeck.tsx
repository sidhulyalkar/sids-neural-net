...
export function FrontierSectionDeck({ items, layoutMode, renderCard, empty }: Props) {
  ...
  const warmIndex = useCallback((index: number, priority: WarmPriority, requestedLimit?: number) => {
    ...
    if (!page) return;
    warmPageMedia(page.items.slice(0, 2), priority, requestedLimit);
  }, [pages]);
  ...
  useEffect(() => {
    ...
    if (idleWindow.requestIdleCallback) {
      const id = idleWindow.requestIdleCallback(run, { timeout: 1400 });
  ...
  useEffect(() => () => {
    ...
    delete document.documentElement.dataset.frontierTurnDirection;
  }, []);

  const navigate = useCallback((nextIndex: number) => {
    if (!pages.length || transitionLocked.current) return;
    ...
    warmIndex(clamped, 'immediate', warmBudget('immediate'));

    const commit = () => flushSync(() => setPageIndex(clamped));
    ...
    document.documentElement.dataset.frontierTurnDirection = direction;
  ...
  const onPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    warmIndex(direction === 'forward' ? activePageIndex + 1 : activePageIndex - 1, 'immediate', warmBudget('immediate'));
  };
  ...
  const pageStyle = { viewTransitionName: 'frontier-page' } as CSSProperties;
  ...
  return (
    <section
      ...
      data-frontier-page-cache="decoded-media-12"
      data-frontier-prefetch-depth="adjacent"
      data-frontier-prefetch-budget="2-intent-2-next-1-prev"
      data-frontier-media-concurrency={MAX_CONCURRENT_MEDIA_WARMS}
      data-frontier-transition="view-transition-single-plane"
      data-frontier-live-planes="1"
    >
      <div className={styles.navBar}>
        ...
        <div className={styles.controls} aria-label="Section navigation">
          <button
            ...
            onPointerEnter={() => warmIndex(activePageIndex - 1, 'immediate', warmBudget('immediate'))}
            onFocus={() => warmIndex(activePageIndex - 1, 'immediate', warmBudget('immediate'))}
        ...
        >
          <div className={layoutMode === 'feed' ? styles.feed : styles.grid}>
            {currentPage.items.map((item, index) => {
              ...
              const renderedCard = renderCard(item, layoutMode);
